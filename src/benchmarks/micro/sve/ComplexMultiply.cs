    ...
    public class ComplexMultiply
    {
        ...
        public unsafe void SveComplexMultiply()
        {
            fixed (uint* a = _source1, b = _source2, c = _result)
            {
                ...
                lmt = Size * 2;
                Vector<uint> pLoop = SveMaskHelper.CreateWhileLessThanMaskUInt32(i, lmt);
                if (Sve.TestFirstTrue(pTrue, pLoop))
                {
                    // Compute the predicate for elements in i + cntw.
                    Vector<uint> pTail = SveMaskHelper.CreateWhileLessThanMaskUInt32(i + cntw, lmt);
        ...
        public unsafe void Sve2ComplexMultiply()
        {
            fixed (uint* a = _source1, b = _source2, c = _result)
            {
                ...
                lmt = Size;
                Vector<ulong> pLoop = SveMaskHelper.CreateWhileLessThanMaskUInt64(i, lmt);
                while (Sve.TestFirstTrue(pTrue, pLoop))
                {
                    ...
                    i += cntd;
                    pLoop = SveMaskHelper.CreateWhileLessThanMaskUInt64(i, lmt);
                }
            }
        }
