...
namespace GC.Infrastructure.Core.Analysis
{
    ...
    {
        public static GCTraceMetrics GetNullItem(string runName, string corerun) =>
            new GCTraceMetrics(runName, corerun);

        private GCTraceMetrics(string runName, string corerun)
        {
        ...
        public GCTraceMetrics(GCProcessData processData, string runName, string configurationName)
        {
            ...
            // 95P
            PauseDurationMSec_95PWhereIsGen0 = GC.Analysis.API.Statistics.Percentile(GoodLinq.Select(GoodLinq.Where(processData.GCs, (gc => gc.Generation == 0)), (gc => gc.PauseDurationMSec)), 0.95);
            PauseDurationMSec_95PWhereIsGen1 = GC.Analysis.API.Statistics.Percentile(GoodLinq.Select(GoodLinq.Where(processData.GCs, (gc => gc.Generation == 1)), (gc => gc.PauseDurationMSec)), 0.95);

            PauseDurationMSec_95PWhereIsBackground = GC.Analysis.API.Statistics.Percentile(GoodLinq.Select(GoodLinq.Where(processData.GCs, (gc => gc.Type == GCType.BackgroundGC)), (gc => gc.PauseDurationMSec)), 0.95);
            ...
            PauseDurationMSec_95PWhereIsBlockingGen2 = GC.Analysis.API.Statistics.Percentile(GoodLinq.Select(GoodLinq.Where(processData.GCs, (gc => gc.Type != GCType.BackgroundGC && gc.Generation == 2)), (gc => gc.PauseDurationMSec)), 0.95);
