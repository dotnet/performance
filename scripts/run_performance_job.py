...
def get_pre_commands(args: RunPerformanceJobArgs, v8_version: str):
    ...
    if args.internal:
        ...
        install_prerequisites += [
            f"{python} -m pip install --user -U pip --force-reinstall",
            f"{python} -m pip install --user urllib3==1.26.18 --force-reinstall",
            f"{python} -m pip install --user azure.storage.blob==12.0.0 --force-reinstall",
            f"{python} -m pip install --user azure.storage.queue==12.0.0 --force-reinstall",
        ]
...
def run_performance_job(args: RunPerformanceJobArgs):
    ...
    if args.dotnet_version_link is not None:
        with urllib.request.urlopen(args.dotnet_version_link) as response:
            ...
            ci_setup_arguments.dotnet_versions = [dotnet_version_check] if dotnet_version_check else [version_check]
