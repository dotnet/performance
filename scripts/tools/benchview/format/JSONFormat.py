...
class Build(object):
    ...
    def __init__(self, repository: str, branch: str, number: str, timestamp: str, buildType: str):
        ...
        if len(repository) > 2000:
            raise ValueError('Build.repository string cannot exceed 2000 characters.')
        self.repository = repository

        if not validators.url(repository):
            raise ValueError('Build.repository is not a valid url format.')

        if not is_valid_name(branch):
            raise ValueError('Build.branch must be a string and it cannot be empty or white space.')
        if len(branch) > 255:
            raise ValueError('Build.branch string cannot exceed 255 characters.')
        self.branch = branch
...
class Configuration(object):
    def __init__(self, displayName: str, properties: dict):
        ...
        if len(properties) < 1:
            raise ValueError('Configuration.properties must have at least one item.')
...
class Metric(object):
    def __init__(self, name: str, unit: str, greaterTheBetter: bool):
    ...
    def __eq__(self, rhs):
        ...
        return same_type and same_name and same_unit and same_greaterTheBetter
...
class Submission(object):
    def __init__(self, name: str, description: str, email: str, cuid: str, created: str):
        ...
        if not validators.email(email):
            raise TypeError('Submission.email field must be a valid email format.')
        self.email = email
