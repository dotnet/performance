...
def get_wmic_os() -> dict:
    ...
    architecture = environ.get('PROCESSOR_ARCHITEW6432')
    dct['architecture'] = environ['PROCESSOR_ARCHITECTURE'] if architecture is None else architecture
    return dct
...
def main() -> int:
    try:
        ...
        if not args['external']:
            ...
            manufacturer = cpu['vendor_id'] if 'vendor_id' in cpu else None
            ...
            if osplatform == "Windows":
                ...
                cores = sysctl_processors()
                threads = sysctl_threads()
                physicalMemory = sysctl_totalmemory() / 1024 / 1024
            else:
                edition = platform.release()
                architecture = platform.uname().machine
        ...
        if condition(args, 'machine_manufacturer'):
            machine_manufacturer = args['machine_manufacturer']
        elif manufacturer is None:
            raise ValueError("Manufacturer is unknown and must be supplied by end user.")
        else:
        ...
        schemaFileName = path.join(scriptDirectory, '..', 'schemas', 'machinedata.json')
        write_object_as_json(args['outfile'], machinedata, schemaFileName)
